...
if(isset($_POST['ccc']))
{
    ...
    mysqli_query($con,"INSERT INTO chat(sid,message,date_time,userid,type) VALUES('$_REQUEST[id]','$_POST[msgd]','$date','$_SESSION[uid]','trainer')");
                           ...
                           <div class="market-update-left">
                              ...
                              <div class="row justify-content-between">
                                 <div class="container">
                                    ...
                                    <!-- Chat Header -->
                                    <?php
                                       $sel = mysqli_query($con, "SELECT * FROM `parent` WHERE id='$_REQUEST[id]'");
                                       while ($row = mysqli_fetch_array($sel)) { ?>
											<div class="chat-header">
												<img class="chat-header-image" src="user.png" alt="Profile Image">
                                    ...
                                    <div class="chat-wrapper" id="message">
                                       <?php
										...
										while ($row = mysqli_fetch_array($result)) {
											...
											if ($row['userid'] == $_SESSION['uid'] && $row['type'] == 'trainer') {
                                    ...
                                    <?php } else { ?>
											<p style="text-align: center; font-size: 18px; color: #777; padding: 20px;">Please select a user to start chatting.</p>
										<?php } ?>
                                 </div>
                              </div>
                           </div>
                     ...
                     <div class="col-md-4 market-update-gd" style="background-color: #deffe3; border-radius: 8px;">
                        <div class="market-update-block">
                           <div class="market-update-left">
                              <h4>Users Section</h4>
                              <?php
                                 $sel = "SELECT DISTINCT userid FROM chat WHERE type='parent'";
								 //echo $sel;
                                 $result = mysqli_query($con, $sel);
                                 while ($row = mysqli_fetch_array($result)) {
                                    $s1 = mysqli_query($con, "SELECT * FROM parent WHERE id='$row[userid]'");
